		    ...
		    size_t * response_len, u2fh_cmdflags flags)
{
  ...
  do
      ...
      for (dev = devs->first; dev != NULL; dev = dev->next)
	{
	  ...
	  rc = send_apdu (devs, dev->id, U2F_AUTHENTICATE, data,
			  ...
			  flags & U2FH_REQUEST_USER_PRESENCE ? 3 : /*UNCOMMENT FOR NO TOUCHLESS SIGNING 7*/ 8, tmp_buf,
			  &len);
	  if (rc != U2FH_OK)
	    {
	      return rc;
	  ...
	  else if ((flags & U2FH_REQUEST_USER_PRESENCE)
	          && (memcmp (tmp_buf, NOTSATISFIED, 2) != 0))
	    {
	      dev->skipped = 1;
	    }
	  else
  ...
  if ((flags & U2FH_REQUEST_USER_PRESENCE)
     && len == 2 && memcmp (buf, NOTSATISFIED, 2) != 0)
    {
      fprintf(stderr, "user authenticator err");
